    ...
    >
      ...
      <nav>
        <ul className="space-y-1 w-full">
         ...
         <Link> <SidebarItem to="/HR/dashboard/departments" label="Departments" isActive={isActive("/dashboard/departments")} onClose={onClose} icon={<AccountBalanceRoundedIcon style={{ color: 'white', fontSize: 20 }} />} /></Link>
          <Link
            to="/dashboard/employees"
            className={`flex items-center justify-between text-white py-2 md:py-1.5 mb-1 hover:bg-blue-600 ${isActive("/dashboard/employees") ? 'bg-sec-color pr-3 rounded-l-3xl' : ''}`}
            onClick={onClose}
          >
            <div className="flex items-center ml-5">
              <span className="mr-2 "><GroupsRoundedIcon style={{ color: 'white', fontSize: 20 }} /></span>
              <span className="text-md ml-1">Employees</span>
            </div>
          </Link>
